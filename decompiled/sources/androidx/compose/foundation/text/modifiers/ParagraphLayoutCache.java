...
public final class ParagraphLayoutCache {
    ...
    private ParagraphLayoutCache(String text, TextStyle style, FontFamily.Resolver fontFamilyResolver, int overflow, boolean softWrap, int maxLines, int minLines) {
        ...
        this.lastDensity = InlineDensity.INSTANCE.m954getUnspecifiedL26CHvs();
        this.layoutSize = IntSizeKt.IntSize(0, 0);
        this.prevConstraints = Constraints.INSTANCE.m5697fixedJhjzzOo(0, 0);
    ...
    public /* synthetic */ ParagraphLayoutCache(String str, TextStyle textStyle, FontFamily.Resolver resolver, int i, boolean z, int i2, int i3, int i4, DefaultConstructorMarker defaultConstructorMarker) {
        this(str, textStyle, resolver, (i4 & 8) != 0 ? TextOverflow.INSTANCE.m5674getClipgIe3tQ8() : i, (i4 & 16) != 0 ? true : z, (i4 & 32) != 0 ? Integer.MAX_VALUE : i2, (i4 & 64) != 0 ? 1 : i3, null);
    }
    ...
    public final void setDensity$foundation_release(Density value) {
        ...
        long newDensity = value != null ? InlineDensity.m946constructorimpl(value) : InlineDensity.INSTANCE.m954getUnspecifiedL26CHvs();
        if (localField == null) {
            this.density = value;
            this.lastDensity = newDensity;
        } else if (value == null || !InlineDensity.m948equalsimpl0(this.lastDensity, newDensity)) {
            this.density = value;
            this.lastDensity = newDensity;
            markDirty();
        }
    ...
    public final void m970setLayoutSizeozmzZPI$foundation_release(long j) {
        this.layoutSize = j;
    }

    /* JADX INFO: renamed from: layoutWithConstraints-K40F9xA, reason: not valid java name */
    public final boolean m969layoutWithConstraintsK40F9xA(long constraints, LayoutDirection layoutDirection) {
        ...
        if (this.minLines > 1) {
            ...
            finalConstraints = localMin.m958coerceMinLinesOh53vG4$foundation_release(constraints, this.minLines);
        } else {
            finalConstraints = constraints;
        }
        boolean z2 = false;
        if (!m967newLayoutWillBeDifferentK40F9xA(finalConstraints, layoutDirection)) {
            if (!Constraints.m5682equalsimpl0(finalConstraints, this.prevConstraints)) {
                ...
                long localSize = ConstraintsKt.m5700constrain4WqzIAM(finalConstraints, IntSizeKt.IntSize(TextDelegateKt.ceilToIntPx(layoutWidth), TextDelegateKt.ceilToIntPx(localParagraph.getHeight())));
                this.layoutSize = localSize;
                if (TextOverflow.m5667equalsimpl0(this.overflow, TextOverflow.INSTANCE.m5676getVisiblegIe3tQ8()) || (IntSize.m5903getWidthimpl(localSize) >= localParagraph.getWidth() && IntSize.m5902getHeightimpl(localSize) >= localParagraph.getHeight())) {
                    z = false;
                }
                this.didOverflow = z;
        ...
        Paragraph it = m966layoutTextK40F9xA(finalConstraints, layoutDirection);
        this.prevConstraints = finalConstraints;
        long localSize2 = ConstraintsKt.m5700constrain4WqzIAM(finalConstraints, IntSizeKt.IntSize(TextDelegateKt.ceilToIntPx(it.getWidth()), TextDelegateKt.ceilToIntPx(it.getHeight())));
        this.layoutSize = localSize2;
        if (!TextOverflow.m5667equalsimpl0(this.overflow, TextOverflow.INSTANCE.m5676getVisiblegIe3tQ8()) && (IntSize.m5903getWidthimpl(localSize2) < it.getWidth() || IntSize.m5902getHeightimpl(localSize2) < it.getHeight())) {
            z2 = true;
        }
        this.didOverflow = z2;
        this.paragraph = it;
    ...
    public final int intrinsicHeight(int width, LayoutDirection layoutDirection) {
        ...
        int result = TextDelegateKt.ceilToIntPx(m966layoutTextK40F9xA(ConstraintsKt.Constraints(0, width, 0, Integer.MAX_VALUE), layoutDirection).getHeight());
    ...
    public final void m971updateL6sJoHM(String text, TextStyle style, FontFamily.Resolver fontFamilyResolver, int overflow, boolean softWrap, int maxLines, int minLines) {
        this.text = text;
        this.style = style;
        this.fontFamilyResolver = fontFamilyResolver;
        this.overflow = overflow;
    ...
    private final Paragraph m966layoutTextK40F9xA(long constraints, LayoutDirection layoutDirection) {
        ParagraphIntrinsics localParagraphIntrinsics = setLayoutDirection(layoutDirection);
        return ParagraphKt.m5125Paragraph_EkL_Y(localParagraphIntrinsics, LayoutUtilsKt.m955finalConstraintstfFHcEY(constraints, this.softWrap, this.overflow, localParagraphIntrinsics.getMaxIntrinsicWidth()), LayoutUtilsKt.m956finalMaxLinesxdlQI24(this.softWrap, this.overflow, this.maxLines), TextOverflow.m5667equalsimpl0(this.overflow, TextOverflow.INSTANCE.m5675getEllipsisgIe3tQ8()));
    }

    /* JADX INFO: renamed from: newLayoutWillBeDifferent-K40F9xA, reason: not valid java name */
    private final boolean m967newLayoutWillBeDifferentK40F9xA(long constraints, LayoutDirection layoutDirection) {
        ...
        if (Constraints.m5682equalsimpl0(constraints, this.prevConstraints)) {
            return false;
        }
        return Constraints.m5689getMaxWidthimpl(constraints) != Constraints.m5689getMaxWidthimpl(this.prevConstraints) || ((float) Constraints.m5688getMaxHeightimpl(constraints)) < localParagraph.getHeight() || localParagraph.getDidExceedMaxLines();
    }
